...
int main()
{
    ...
    std::shared_ptr<AbstractObserver> serverLogin = std::make_shared<ServerA>();
    std::shared_ptr<AbstractObserver> serverRegister = std::make_shared<ServerB>();
    server.RegisterObserve(serverLogin);
    server.RegisterObserve(serverRegister);
    server.Run();
    return 0;
}