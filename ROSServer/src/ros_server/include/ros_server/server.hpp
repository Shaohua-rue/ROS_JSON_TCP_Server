...
public:
    ...
    void RegisterObserve(std::shared_ptr<AbstractObserver> obs)
    {
        sub->registerObserve(obs);
    }
    void RemoveObserve(std::shared_ptr<AbstractObserver> obs)
    {
        sub->removerObserve(obs);
    }
    void Run()
    {
        while(1)
        {
            ...
            if (nfds ==  -1)
            {
                std::cout << "Process Exits!" << std::endl;
                break;
            }
            else if(nfds == 0)
            {
                    ...
                    if(events[i].events & EPOLLIN)
                    {
                        ...
                        if (!connection_closed && recv_len > 0) {
                            ...
                            if(header_size > 0 && recvBuf.size()>= header_size)
                            {
                                ...
                                sub->notify(funcName,recvArgMap);
                                header_size = 0;
                            }
